...
func run(cmd *cobra.Command, args []string) {
	...
	//go process.Get100Qi(mysql.DB)
	//model.Get100PaiXu3(mysql.DB)
	//process.GetTodayResultForPX3(mysql.DB)
	go process.FuCai3D(mysql.DB)
	go process.PaiXu3(mysql.DB)
	//go process.BlockCheckFromTo(mysql.DB, redis.Rdb)
	router.Setup()
}
